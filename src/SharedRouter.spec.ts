...
describe('Common Router Spec', () => {
  for (const { routerName, Router } of RoutersToTest) {
    describe(`ROUTER = ${routerName}`, () => {
      const router = Router()
      ...
      describe(`.fetch({ method = 'GET', url })`, () => {
        it('always returns a Promise', () => {
          const syncRouter = Router()
          syncRouter.get('/foo', () => 3)

          const response = syncRouter.fetch(toReq('/foo'))
        ...
        it('match earliest routes that match', async () => {
          ...
          await router.fetch(toReq('/foo/static'))
          expect(handler1).toHaveBeenCalled()
          expect(handler2).not.toHaveBeenCalled()

          await router.fetch(toReq('/foo/3'))
        ...
        it('honors correct method (e.g. GET, POST, etc)', async () => {
          ...
          await router.fetch(toReq('POST /foo'))
        ...
        it('allows missing handler later in flow with "all" channel', async () => {
          ...
          router1.all('/nested/*', router2.fetch).all('*', missingHandler)

          await router1.fetch(toReq('/foo'))
          expect(missingHandler).toHaveBeenCalled()

          await router1.fetch(toReq('/nested/foo'))
          expect(matchHandler).toHaveBeenCalled()
        })

        it(`won't throw on unknown method`, () => {
          expect(() =>
            router.fetch({ method: 'CUSTOM', url: 'https://example.com/foo' })
          ).not.toThrow()
        })
        ...
        it('can accept a basepath for routes', async () => {
          ...
          await router.fetch(toReq('/api/foo/13'))
        ...
        it('allows any method to match an "all" route', async () => {
          ...
          await router.fetch(toReq('POST /crud/bar'))
          expect(handler).toHaveBeenCalledTimes(2)
        })
        ...
        it('stops at a handler that throws', async () => {
          ...
          await router.fetch(toReq('/foo')).catch(escape)
        ...
        it('can throw method not allowed error', async () => {
          ...
          let response = await router.fetch(createRequest('post'))
          ...
          response = await router.fetch(createRequest('put'))
      ...
      describe(`.fetch({ method = 'GET', url }, ...args)`, () => {
        it('passes extra args to each handler', async () => {
          ...
          await r.fetch(req, originalA, originalB)
        ...
        it('can handle POST body even if not used', async () => {
          ...
          await router.fetch(createRequest('put'))
          expect(errorHandler).toHaveBeenCalled()

          const response = await router.fetch(createRequest('post'))
          expect(handler).toHaveBeenCalled()
      ...
      describe('MIDDLEWARE', () => {
        it('calls any handler until a return', async () => {
          ...
          const results = await router.fetch(toReq('/'))
